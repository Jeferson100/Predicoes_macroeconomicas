...
def test_escalando_dados():
    data = pd.DataFrame({"A": [1, 2, 3, 4, 5], "B": [5, 4, 3, 2, 1]})
    scaled_data, _ = escalando_dados(data, tipo="minmax")
    assert scaled_data.min() == 0
    assert scaled_data.max() == 1
    scaled_data, _ = escalando_dados(data, tipo="standard")
