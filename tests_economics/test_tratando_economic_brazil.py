from Modelo_Previsao_Taxa_Selic.tratando_economic_brazil import (
    tratando_dados_bcb,
...
def test_tratando_dados_ibge_link():
    dados = tratando_dados_ibge_link(coluna="pib", link="https://sidra.ibge.gov.br/geratabela?format=xlsx&name=tabela5932.xlsx&terr=N&rank=-&query=t/5932/n1/all/v/6561/p/all/c11255/90707/d/v6561%201/l/v,p%2Bc11255,t")
    assert isinstance(dados.index, pd.DatetimeIndex)
...
def test_tratando_metas_inflacao():
    ...
    assert isinstance(dados.index, pd.DatetimeIndex)
    assert ["meta_inflacao", "inflacao_efetiva", "diferenca_meta_efetiva"] == list(dados.columns) 
